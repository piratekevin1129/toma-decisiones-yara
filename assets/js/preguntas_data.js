var preguntas_data = [
    {
        id:1,
        size:'normal',
        vimeo:'519127935',
        pregunta:'¿Qué proveedor debe elegir Juan?',
        opcion1:'El proveedor X está disponible de inmediato, se compromete a  cumplir con los requisitos HESQ de Yara en los próximos meses y es la oferta más económica.',
        opcion2:'El proveedor Y está disponible de inmediato y tiene buenas referencias de mercado en términos de seguridad. Cuenta con personal sin experiencia en trabajo en alturas.',
        opcion3:'El proveedor Z tiene disponibilidad luego de una semana, es la oferta más costosa, pero cuenta con estándares de seguridad robustos ajustándose a los requisitos HESQ de YARA.',
        correcto:3,
        video:'1.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Estableciendo prioridades para reducir los riesgos a niveles aceptables.',
                'Y asignando a la persona adecuada para el puesto idóneo en el momento oportuno'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'Debemos mejorar porque es tu responsabilidad promover los recursos necesarios para que se cumplan los lineamientos de HESQ, reglas de oro y/o estándares técnicos (HOPS y TOPS). <span>Así estas decisiones nos tomen más tiempo. !Primero la seguridad de todos!</span>'
        }
    },
    {
        id:2,
        size:'normal',
        vimeo:'522315234',
        pregunta:'¿Qué debería hacer Carolina?',
        opcion1:'Parar de inmediato el cargue de camiones y coordinar en conjunto con el supervisor de HESQ la limpieza del área y el retiro de residuos peligrosos al área correspondiente.',
        opcion2:'Escalar por correo electrónico al Site Manager y al Supervisor del Sitio. Esperar que la información sea revisada durante la semana.',
        opcion3:'Realizar personalmente el movimiento de los residuos peligrosos sin contar con el permiso de trabajo',
        correcto:1,
        video:'2.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Evaluando los resultados de seguridad de una manera sincera y abierta',
                'Además de detener los trabajos cuando el riesgo a la seguridad se considera inaceptable'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'<span>Debes mejorar tu enfoque como líder.</span> Puedes detener trabajos cuando el riesgo de seguridad sea inaceptable. Todos hacemos parte de la seguridad y tu rol de liderazgo impacta la efectividad de acciones preventivas y/o correctivas.'
        }
    },
    {
        id:3,
        size:'normal',
        vimeo:'519129772',
        pregunta:'Si usted fuera el supervisor inmediato de Martin ¿cómo podría reconocer sus ideas?',
        opcion1:'Identificar cuáles ideas generan prevención de seguridad y salud en el trabajo y felicitarlo por sus logros. ',
        opcion2:'Identificar e incentivar activamente a los equipos, creando espacios para reconocer las ideas que generan impacto positivo en la reducción de riesgos y mejora de la productividad',
        opcion3:'Premiar cada idea que se genere sin importar el impacto para el negocio.',
        correcto:2,
        video:'3.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Promoviendo espacios que generen ideas de prevención de riesgos que mejoren los resultados. ',
                'Y celebrando y premiando los éxitos en seguridad que generen impactos positivos en la reducción de riesgos, mejoras en la calidad y productividad'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'Debes mejorar porque como líder debes propiciar espacios para que se generen ideas en prevención de riesgos y reconocer aquellas que generen impactos positivos en la reducción de riesgos, mejoras en la calidad y productividad. <span>El logro de cero accidentes en Yara requiere de la contribución de todos</span>'
        }
    },
    {
        id:4,
        size:'large',
        vimeo:'522058330',
        pregunta:'¿Cuál sería la mejor acción que debería promover María entre sus colegas para cumplir con los objetivos de trabajo diario?',
        opcion1:'María deberá organizar y participar en inspecciones pre-operativas e identificación de peligros y evaluación de riesgos que se realizan.',
        opcion2:'María deberá basar sus decisiones en hechos objetivos. Sin embargo, en alta temporada o cuando su jefe inmediato lo solicite deberá operar sin realizar inspecciones previas debido a que la operación es muy estable y no ha generado incidentes en 2021',
        opcion3:'María deberá evaluar solo los riesgos críticos de su operación y los de bajo riesgo',
        correcto:1,
        video:'4.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Basando tus decisiones en hechos objetivos.',
                'Y haciendo parte de la verificación e implementación de mejoras de las condiciones de trabajo de tu equipo'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'<span>Debes mejorar</span>, porque tu liderazgo en Yara se alinea con la curiosidad y tú debes hacer parte de la verificación e implementación de mejoras de las condiciones de trabajo de tu equipo. <span>Tu ejemplo educa a todos los colegas.</span>'
        }
    },
    {
        id:5,
        size:'normal',
        vimeo:'519131710',
        pregunta:'Si usted estuviera en el lugar de Janeth ¿Cómo resolvería la situación de seguridad que requiere atención inmediata en picos altos de producción?',
        opcion1:'Janeth deberá concentrarse en la producción y asignar un equipo interdisciplinario para que en conjunto puedan atender la emergencia y cerrar la situación en el menor tiempo posible.',
        opcion2:'Janeth deberá suspender la operación y evaluar las condiciones que llevaron a los hechos de emergencias y convocar una reunión con todos los líderes',
        opcion3:'Janeth deberá apersonarse de la situación de emergencia, revisar y asignar las responsabilidades y actividades a quien le corresponda, dejando los hallazgos a debatir en la próxima reunión de equipo',
        correcto:3,
        video:'5.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Disponible cuando te necesiten',
                'Contar con mente abierta y respetar las opiniones de los demás.',
                'Y manteniendo la calma, dando órdenes y resolviendo situaciones que requieren atención inmediata tales como emergencias, incidentes, condiciones de riesgo, etc'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'<span>Debes mejorar</span>, porque en las situaciones desafiantes de emergencias, incidentes, condiciones de riesgo, etc. El lider actúa con calma, da órdenes y resuelve la situación apersonándose para encontrar soluciones. Como líder desarrollas mentalidad abierta y respetas las opiniones de los demás'
        }
    },
    {
        id:6,
        size:'large',
        vimeo:'519132757',
        pregunta:'¿Cómo le ayudarías a Andres a mejorar la gestión HESQ y la productividad con las ideas dadas en los espacios de participación?',
        opcion1:'Andrés debe fomentar la colaboración entre áreas, animando a los diferentes equipos a hablar y proponer soluciones y mejoras en la gestión Integral HESQ y operaciones, solo escogiendo las que sean dadas por líderes.',
        opcion2:'Andrés debe trabajar en equipo con las diferentes áreas, logrando identificar líderes de gestión que puedan facilitar la implementación de mejoras eficientes y eficaces alineadas a los objetivos de la compañía.',
        opcion3:'Andrés tiene muy buenas intenciones considerando todas las ideas aportadas en los espacios de participación, sin importar que sean demasiadas y demande mucho tiempo',
        correcto:2,
        video:'6.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Fomentando la colaboración entre departamentos',
                'Animando al equipo a hablar abiertamente de las cuestiones de seguridad.',
                'Y creando espacios de participación donde los colaboradores pueden aportar ideas para mejorar la gestión de HESQ y la Productividad'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'Debemos mejorar porque es tu responsabilidad promover espacios de participación buscando <span>mejorar la gestión HESQ y la productividad de la compañía</span> y aunque lo estés realizando debes velar porque se dé la colaboración entre los departamentos y que todas estas ideas y propuestas dadas puedan ser focalizadas, priorizadas y analizadas para su posible implementación. '
        }
    },
    {
        id:7,
        size:'large',
        vimeo:'519133824',
        pregunta:'¿Cómo reaccionaría Lira con la información dada por Jose un día después de lo sucedido?',
        opcion1:'Lira se muestra inconforme con lo sucedido, porque ella valora los trabajos hechos con seguridad, no le dice nada a Jose delante de sus compañeros, pero a solas le comenta lo que representa no informar a tiempo',
        opcion2:'Lira se da cuenta que los espacios están cumpliendo su objetivo y toma lo sucedido con Jose como un caso oportuno de aprendizaje para todos, incentiva a cumplir con los reportes de incidentes a tiempo e inmediatamente analiza las posibles causas para como equipo implementar acciones.',
        opcion3:'Lira analiza que pudo pasar para que Jose no hubiese reportado, reporta el incidente  habla con el área HESQ Y analizan en conjunto acciones para promover los reportes y aquellas que eviten la ocurrencia',
        correcto:2,
        video:'7.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Haciendo preguntas a tu equipo de trabajo sobre las acciones que podemos implementar y resolver dudas para cumplir los objetivos de la estrategia del área de HESQ.'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'Debes mejorar porque tu liderazgo también se ve activo cuando haces preguntas a tu equipo sobre las acciones a implementar y cuando resuelves  dudas para cumplir los objetivos de la estrategia del área y sin incidentes. <br /><span></span>Tu liderazgo se ve reflejado en saber comunicar con claridad y objetividad</span>'
        }
    },
    {
        id:8,
        size:'normal',
        vimeo:'519134863',
        pregunta:'¿Qué debe hacer Diana con la situación presentada, teniendo en cuenta el desarrollo de los valores de Yara?',
        opcion1:'Diana realiza la consulta en la recepción. Como no es muy conocedora del tema habla con HESQ y reporta el caso, quienes además aprovechan lo sucedido para dar una charla al personal del área.',
        opcion2:'Diana va a la recepción, habla con la encargada, consulta a quien pertenece la caja y espera a que sea removida',
        opcion3:'Diana muy proactiva decide rodar un poco la caja y despejar el extintor de este obstáculo, levanta la caja que es pesada y la cambia de sitio',
        correcto:1,
        video:'8.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Preguntando, investigando e interesandote por aprender de sus pares u otras áreas, para identificar y aplicar las buenas prácticas y/o detectar oportunidades de mejora en HESQ'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'Debes seguir desarrollando el valor de la curiosidad en ti y en tu equipo de trabajo, no limitarte a pasar, mirar y observar y no comunicar y/o reportar posibles mejoras en la gestión HESQ, independiente del área al que pertenezca.'
        }
    },
    {
        id:9,
        size:'normal',
        vimeo:'519135639',
        pregunta:'Si estuvieras en el lugar de Antonio, ¿Cuál sería la principal acción que realizarías en seguridad?',
        opcion1:'Antonio se reúne con el equipo líder y decide como estrategia que todos deben estar tan pendiente de los trabajadores como si fueran guardias de seguridad',
        opcion2:'Antonio se reúne con el equipo líder y deciden salir a las diferentes áreas y conversar con todos los colaboradores, buscando obtener una lluvia de ideas que le ayuden a implementar acciones.',
        opcion3:'Antonio se reúne con el área HESQ y acuerdan una estrategia.',
        correcto:2,
        video:'9.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Tomando acciones necesarias para garantizar un ambiente de trabajo seguro.',
                'Y realizando seguimiento a la implementación efectiva de acciones originadas de condiciones inseguras, incidentes, inspecciones y auditorías.'
            ]
        },
        mal:{
            title:'¡Ups!',
            msg:'Debes mejorar la forma de visibilizar el liderazgo debido a que, aunque es un buen camino tener una estrategia de Gestión, queremos verte a ti liderar con el ejemplo.'
        }
    },
    {
        id:10,
        size:'large',
        vimeo:'519136956',
        pregunta:'¿Cómo puedes mejorar este tipo de comunicaciones?',
        opcion1:'Todas las comunicaciones y lecciones aprendidas derivadas de temas HESQ deben ser comunicadas con prioridad en toda la organización',
        opcion2:'Debo comunicar la información a los líderes para que ellos realicen el despliegue a sus equipos.',
        opcion3:'Debo garantizar en conjunto con mi equipo de trabajo que este tipo de situaciones no se vuelvan a presentar, proponiendo una estrategia de comunicaciones que asegure el cubrimiento de la población de interés',
        correcto:3,
        video:'10.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'Buscando el aprendizaje y transmitirlo a los compañeros y al equipo.',
                'Además de comunicarte de manera transparente, entregando mensajes claros y proporcionando comentarios constructivos para prevenir situaciones similares en el futuro'
            ]
        },
        mal:{
            title:'Es importante que',
            msg:'Asegures que todos los temas de importancia sean cubiertos dentro de los trabajadores, entregando mensajes claros y oportunos que nos ayuden a prevenir eventos en el futuro. Busquemos las estrategias para que nos puedan dar los mejores resultados.'
        }
    },
    {
        id:11,
        size:'large',
        vimeo:'519137720',
        pregunta:'¿Qué proceso debe seguir Jair para que el arreglo se realice a la brevedad posible y nadie resulte lesionado?',
        opcion1:'Contratar inmediatamente a un proveedor que ha realizado trabajos anteriores en el site, sin una verificación previa de los requisitos HESQ.',
        opcion2:'Convocar a una reunión extraordinaria con su equipo primario, incluido HESQ para realizar el análisis de la actividad, establecer el procedimiento, identificar los peligros evaluar los riesgos, determinar los controles pertinentes, los recurso/materiales y los requisitos que el o los contratistas deben cumplir',
        opcion3:'Pedir al supervisor de HESQ que se encargue de todo el proceso (contratación, validación de documentación, identificación de riesgos, etc.) y le informe cuando esté terminado el trabajo',
        correcto:2,
        video:'11.mp4',
        bien:{
            title:'¡Muy bien! debes seguir:',
            objetivos:[
                'priorizaste la seguridad del equipo promoviendo el cumplimiento de controles asociados a las Reglas de Oro',
            ]
        },
        mal:{
            title:'Como líder es importante que',
            msg:'Te asegures de involucrar a tu equipo en la identificación de riesgos inherentes en una actividad, más aún al asociarse a una Regla de Oro.'
        }
    }
]


var resultados_data = [
    {
        texto:'Sigue así y vamos... enseña a otros.',
        intro:'Felicitaciones. Tus acciones como líder nos permiten concluir que:',
        misiones:[
            'Tomas la responsabilidad, obtienes los hechos y entiendes el impacto de tu propia decisión',
            'Mejoras la forma de trabajo',
            'Desarrollas a tu equipo. ',
            'Eres transparente, entregas mensajes claros y proporcionas comentarios constructivos.',
            'Compartes información y apoyas a tu equipo en el trabajo.',
            'Haces preguntas audaces y escucha activamente.',
            'Exploras nuevas soluciones y formas de trabajar. ',
            'Y demuestras interés en aprender de los demás y de aplicar las mejores prácticas.'
        ]
    },
    {
        texto:'Vamos por buen camino',
        intro:'Debemos seguir fortaleciendo tus habilidades y trabajar en los siguientes puntos para enfocar tus resultados de seguridad:',
        misiones:[
            'Tomando la responsabilidad, obteniendo los hechos y entiendo el impacto de tu propia decisión',
            'Siendo transparente, entregando mensajes claros y proporcionando comentarios constructivos.',
            'Compartiendo información y apoyando a tu equipo en el trabajo.',
            'Haciendo preguntas audaces y escuchando activamente.',
            'Explorando nuevas soluciones y formas de trabajar.',
            'Y demostrando interés en aprender de los demás y de aplicar las mejores prácticas.'
        ]
    },
    {
        texto:'Debemos mejorar y tu equipo de HESQ está alerta para ayudarte.',
        intro:'Te invitamos hacer un alto en el camino y enfocarte en nuestras competencias para:',
        misiones:[
            'Tomar la responsabilidad, obteniendo los hechos y entendiendo el impacto de tu propia decisión',
            'Mejorar la forma de trabajo',
            'Desarrollarte a ti mismo y a los demás',
            'Ser transparente, entregando mensajes claros y proporcionando comentarios constructivos.',
            'Compartir información y apoyar a tu equipo en el trabajo.',
            'Hacer preguntas audaces y escuchar activamente.',
            'Explorar nuevas soluciones y formas de trabajar. ',
            'Y demostrar interés en aprender de los demás y de aplicar las mejores prácticas.'
        ]
    }
]